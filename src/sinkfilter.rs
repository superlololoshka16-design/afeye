...
pub struct SinkFilterStats {
    ...
    /// distinct executed-bytecode values that matched byte-exact inside a
    /// sink payload (Aho-Corasick). A value-provenance FACT.
...
pub fn run(
    collect_dir: &Path,
    valuebook: &HashMap<Vec<u8>, (crate::bctrace::ValueOrigin, u64)>,
    ...
    for r in &recs {
        ...
        // egress tags from 0029 (wt-stream-out, wt-datagram-out,
        // rtc-datachannel-out) and 0030 (webgpu/write-buffer,
        // webgpu/write-texture): tokens leaving through these channels
        // are sinks too, not just carriers. ws-frame-in / *-in stay
        // carriers (inbound).
        let is_ws_sink = r.kind == "websocket"
            && (tag == "ws-frame-out"
                || tag == "wt-stream-out"
                || tag == "wt-datagram-out"
                || tag == "rtc-datachannel-out"
                || tag == "webgpu/write-buffer"
                || tag == "webgpu/write-texture");
    ...
    // ---- value provenance (LOW-LEVEL, replaces window/hash guessing) ----
    // Patterns: byte sequences that executed bytecode instructions really
    // carried (0033 valuebook: acc/reg strings + cp literals). Text: every
    // sink payload + every carrier payload. One Aho-Corasick pass per
    // payload. A hit = this exact value was observed inside a boundary-
    // crossing record. Fact, not correlation.
    let mut ac = crate::valueflow::AhoCorasick::new();
    let mut ac_origins: Vec<(crate::bctrace::ValueOrigin, u64)> = Vec::new();
    for (bytes, origin) in valuebook {
        // keep values bounded: skip pathological >4KiB strings, they are
        // not tokens, they are bulk data the sink already carries whole
        if bytes.len() > 4096 {
            continue;
        }
        let id = ac.insert(bytes);
        while ac_origins.len() <= id as usize {
            ac_origins.push((
                crate::bctrace::ValueOrigin {
                    func_id: 0,
                    off: 0,
                    op: String::new(),
                    src: "",
        ...
        ac_origins[id as usize] = origin.clone();
    }
    ac.build();

    // value_hits: script_name -> {value_len, sample hex head, sink kinds}
    let mut value_hits: BTreeMap<String, Vec<serde_json::Value>> =
        BTreeMap::new();
    let mut value_hit_values: HashSet<Vec<u8>> = HashSet::new();
    if !ac.is_empty() {
        for r in recs.iter().filter(|r| payload_kinds.contains(&r.kind.as_str())) {
            let payload = match read_payload(&raw_dir, r) {
                Some(p) => p,
                 None => continue,
            };
            let (_tag, body) = name_of(&payload);
            if body.is_empty() {
                continue;
            }
            ac.find(&body, |pid, off| {
                let val = ac.pattern(pid as usize);
                let origin = &ac_origins[pid as usize];
                let script = func_scripts
                    .get(&origin.0.func_id)
                    .cloned()
                    .unwrap_or_else(|| format!("func:{:08x}", origin.0.func_id));
                value_hit_values.insert(val.to_vec());
    ...
    stats.value_proven_values = value_hit_values.len() as u64;
    ...
    let report = json!({
        ...
        "value_provenance": {
            ...
            "rule": "LOW-LEVEL value identity, not window/hash guessing. Patterns = byte sequences that executed Ignition instructions actually carried (0033 trace: accumulator strings, register strings, constant-pool literals). Text = every sink/carrier payload. One Aho-Corasick pass per payload. A hit = that exact value was observed inside a boundary-crossing record at byte offset N, sourced from bytecode offset B of func F (op O, src acc|reg|cp). Facts only: no time window, no blake3 window, no stride alignment, no hop cap.",
        },
        "graph": {
...
mod tests {
    ...
    fn e2e_token_chain_verdicts() {
        ...
        let sf = run(&collect_dir, &HashMap::new(), &HashMap::new()).expect("sinkfilter run");
    ...
    #[test]
    fn value_provenance_matches_executed_value_in_sink() {
        // LOW-LEVEL value identity: a value an executed bytecode instruction
        // carried (here a cp literal "TOKEN-abc123-xyz") is found byte-exact
        // inside a net-request req-body sink by Aho-Corasick. No window, no
        // blake3, no stride - the bytes literally occur.
        let tmp = tempfile::tempdir().unwrap();
        let raw_dir = tmp.path().join("raw");
        let collect_dir = tmp.path().join("collect");
        std::fs::create_dir_all(&raw_dir).unwrap();

        let value = b"TOKEN-abc123-xyz";
        let mut rec: Vec<u8> = Vec::new();
        let t0: u64 = 1_000_000_000;
        {
            let mut w = std::io::Cursor::new(&mut rec);
            let mut put = |kind: u8, ts: u64, payload: &[u8]| {
                let total: u32 = (16 + payload.len()) as u32;
                w.write_all(&total.to_le_bytes()).unwrap();
                w.write_all(&[kind, 0, 0, 0]).unwrap();
                w.write_all(&ts.to_le_bytes()).unwrap();
                w.write_all(payload).unwrap();
            };
            put(0, t0, b"afeye-sink/v8 v2 pid=1");
            // script-source so a chain exists for the report
            put(1, t0 + 1_000_000, b"https://af.io/build.js\0function b(){return 1}");
            // net-request req-body sink carrying the value at a non-aligned shift
            let mut rb = Vec::new();
            rb.extend_from_slice(b"req-body");
            rb.push(0);
            rb.extend_from_slice(b"envelope:"); // 9-byte prefix, defeats 16-stride windows
            rb.extend_from_slice(value);
            rb.extend_from_slice(b":tail");
            put(17, t0 + 5_000_000, &rb);
        }
        std::fs::write(raw_dir.join("v8-99.rec"), &rec).unwrap();
        ...
        // valuebook as bctrace would build it: the value came from a cp
        // literal of function 0xabcd1234, which belongs to build.js.
        let mut valuebook: HashMap<Vec<u8>, (crate::bctrace::ValueOrigin, u64)> = HashMap::new();
        valuebook.insert(
            value.to_vec(),
            (
                crate::bctrace::ValueOrigin {
                    func_id: 0xabcd1234,
                    off: 7,
                    op: "LdaConstant".to_string(),
                    src: "cp",
                },
                1,
            ),
        );
        let mut func_scripts: HashMap<u32, String> = HashMap::new();
        func_scripts.insert(0xabcd1234, "https://af.io/build.js".to_string());

        let sf = run(&collect_dir, &valuebook, &func_scripts).expect("run");
        assert_eq!(sf.value_proven_values, 1, "value not matched byte-exact");
        assert_eq!(sf.value_proven_scripts, 1);

        let rep: serde_json::Value = serde_json::from_slice(
            &std::fs::read(collect_dir.join("filtered/report.json")).unwrap(),
        )
        .unwrap();
        let vp = &rep["value_provenance"];
        assert_eq!(vp["distinct_values_matched"].as_u64(), Some(1));
        let hits = vp["by_script"]["https://af.io/build.js"].as_array().unwrap();
        assert_eq!(hits.len(), 1);
        assert_eq!(hits[0]["kind"], "net-request");
        assert_eq!(hits[0]["src"], "cp");
        assert_eq!(hits[0]["op"], "LdaConstant");
        assert_eq!(hits[0]["val_len"].as_u64(), Some(value.len() as u64));
        // matched at byte offset 9 inside the body (after "envelope:")
        assert_eq!(hits[0]["off_in_payload"].as_u64(), Some(9));
    }

    #[test]
    fn value_provenance_no_false_positive_on_absent_value() {
        // The valuebook carries a value that does NOT occur in any sink
        // payload -> zero matches. Proves the matcher is exact, not fuzzy.
        let tmp = tempfile::tempdir().unwrap();
        let raw_dir = tmp.path().join("raw");
        let collect_dir = tmp.path().join("collect");
        std::fs::create_dir_all(&raw_dir).unwrap();
        ...
        let t0: u64 = 1_000_000_000;
        {
            ...
            rb.extend_from_slice(b"req-body");
            rb.push(0);
            rb.extend_from_slice(b"totally-different-payload-bytes");
            put(17, t0 + 5_000_000, &rb);
        }
        std::fs::write(raw_dir.join("v8-98.rec"), &rec).unwrap();
        let collector =
            crate::collect::Collector::spawn_dirs(raw_dir.clone(), collect_dir.clone());
        std::thread::sleep(std::time::Duration::from_millis(600));
        let _ = collector.stop();
        ...
        let sf = run(&collect_dir, &valuebook, &HashMap::new()).expect("run");
        assert_eq!(sf.value_proven_values, 0, "false positive match");
    }
}
