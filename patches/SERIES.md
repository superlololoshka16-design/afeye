...
wire-формат v2: payload-блоки [u8 tag][u16 len][bytes], теги: 0=acc-строка, 1=acc-f64-биты, 2=acc-smi, 3+k=строка регистра regs[k]. flags bits5-7 = число регистров (Rust режет [0,n_regs), НЕ скипает нулевые слова — Smi 0 тегается в word 0, скип съезжал атрибуцию строк). регистровые операнды декодятся движковым DecodeRegisterOperand (знаковый; unsigned-чтение превращало r0=-1 в 255 и читало мусор), RegPair/RegList дают ВСЕ свои регистры.
...
ОГРАНИЧЕНИЯ честно: (1) без --jitless горячая функция уйдёт в Maglev/TurboFan и перестанет попадать в трейс — jitless ОБЯЗАТЕЛЕН для полного слепого пятна, цена — производительность, её закрывает 0034. (2) per-instruction строки функций НЕ снимаются (line в func-def, на старте функции) — per-offset линии декодируются оффлайн из source position table если понадобятся. (3) значения регистров — сырые слова + payload только для String/HeapNumber/Smi аккумулятора; произвольные HeapObject дампаются словом (адрес), не содержимым. (4) 50M cap при реальном хот-лупе PoW дойдёт — ring переполнится, drop-witness (0023) покажет, не молча.
