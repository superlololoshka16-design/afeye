...
0038 sink fork-guard + drop-witness точность (два C++-бага подтверждённых кодом в аудитах):
(1) FORK-GUARD: ReinitAfterFork в v8 (0001) и net (0011) НЕ имел guard if(!g_on) — blink (0005) имел. Enabled() при смене pid звал ReinitAfterFork даже когда AFEYE_SINK не задан, detached DrainLoop открывал .rec и писал sink-hello. Следствие: детектор sink_alive в main.rs (v8/sink-hello + blink/sink-hello + net/sink-hello) рапортовал true для НЕ-afeye хрома — ложный positive capture-integrity, весь ран помечался «chrome пропатчен» когда sink фактически выключен. Фикс: guard добавлен в v8+net, parity с blink — при выключенном sink только g_pid.store, без потока.
(2) DROP-WITNESS: WriteDropReport(fd, now_dropped) где now_dropped = g_dropped.load() ТОЛЬКО, но SinkDropped() = g_dropped + g_write_err. fd-ошибки записи не засвидетельствованы — kind-39 witness занижал потери, sinkfilter drops_witnessed() и dead_end_proven доверяли неполному witness (цепь помечалась dead-end-proven хотя часть записей потерялась на записи в fd, не на ring). Фикс во ВСЕХ ТРЁХ sink'ах: now_dropped = g_dropped + g_write_err, witness совпадает с SinkDropped(). Ставит поверх 0001/0005/0011/0023/0033, git apply --check зелёный.
